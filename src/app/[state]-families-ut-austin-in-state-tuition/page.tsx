    ...
    <>
      ...
      <header className="bg-navy-dk text-paper py-20">
        <div className="narrow">
          <p className="eyebrow !text-gold-lt">For families relocating from {s.name}</p>
          <h1 className="!text-paper max-w-[22ch]">UT Austin in-state tuition for {s.name} families</h1>
          <p className="text-white/80 text-lg max-w-[60ch] mt-4">
      ...
      <section className="py-12">
        <div className="narrow">
          ...
          {s.residencyMoveNote && (<>
            ...
            <p className="text-sm text-mid">This is general information, not legal or tax advice. Consult a Texas-licensed attorney or CPA before relying on anything specific.</p>
          </>)}
          ...
          {s.faq.length > 0 && (<>
            ...
            {s.faq.map(({ q, a }, i) => (
              <details key={i} className="border-b border-border py-4">
                <summary className="cursor-pointer font-serif text-lg text-navy font-semibold">{q}</summary>
                <div className="pt-3 text-mid">{a}</div>
              </details>
            ))}
          </>)}
          ...
          <div className="bg-cream p-7 rounded mt-9">
            ...
            <p className="text-mid">If you are weighing a Texas property purchase as part of the pathway, you can talk to our recommended Austin broker. Free, no obligation.</p>
